...
import com.nonameradio.app.core.utils.AsyncExecutor;
...
public class StationActions {
    ...
    private static void retrieveAndCopyStreamUrlToClipboard(final @NonNull Context context, final @NonNull DataRadioStation station) {
        ...
        AsyncExecutor.submitIOTask(() -> {
            ...
            Log.e(TAG, "Error in retrieve stream URL task", throwable);
            AsyncExecutor.runOnMainThread(() -> {
                Context ctx = contextRef.get();
                if (ctx != null) {
                    EventBus.post(HideLoadingEvent.INSTANCE);
    ...
    public static void share(final @NonNull Context context, final @NonNull DataRadioStation station) {
        ...
        AsyncExecutor.submitIOTask(() -> {
            ...
            // Handle any exceptions that occurred during the task
            Log.e(TAG, "Error in share station task", throwable);
            AsyncExecutor.runOnMainThread(() -> {
                Context ctx = contextRef.get();
                if (ctx != null) {
                    EventBus.post(HideLoadingEvent.INSTANCE);
    ...
    private static void vote(final @NonNull Context context, final @NonNull DataRadioStation station) {
        ...
        AsyncExecutor.submitIOTask(() -> {
            ...
            // Handle any exceptions that occurred during the vote task
            Log.e(TAG, "Error in vote task", throwable);
            return null;
        });
    }
}
