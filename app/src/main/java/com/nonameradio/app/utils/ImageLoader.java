...
public class ImageLoader {
    ...
    public static void loadStationIcon(Context context, String url, ImageView imageView, @Nullable Drawable placeholder) {
        ...
        RequestOptions options = createStationIconOptions(context, safePlaceholder)
                .timeout(3000) // Быстрый таймаут для списков станций
                .skipMemoryCache(false) // Использовать memory cache
                .onlyRetrieveFromCache(false); // Сначала попробовать сеть

        Glide.with(context)
                .load(url)
                .apply(options)
                .thumbnail(0.25f) // Загружать миниатюру 25% сначала (быстрее чем 0.1f)
                .into(imageView);
    }
    ...
    private static RequestOptions createStationIconOptions(Context context, Drawable placeholder) {
        // Ограничить размер для ускорения декодирования
        final int iconSize = dpToPx(context, 48); // Фиксированный размер 48dp
        
        Transformation<Bitmap> transformation;
        if (Utils.useCircularIcons(context)) {
            transformation = new MultiTransformation<>(new CenterCrop(), new CircleCrop());
        } else {
            transformation = new MultiTransformation<>(new CenterCrop(), new RoundedCorners(dpToPx(context, 12)));
        }

        return createRequestOptions(context, placeholder, transformation)
                .override(iconSize, iconSize); // Ограничить размер декодированного изображения
    }
    ...
    private static RequestOptions createRequestOptions(Context context, Drawable placeholder, Transformation<Bitmap> transformation) {
        ...
        RequestOptions options = new RequestOptions()
                ...
                .diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                .encodeQuality(80) // Снизить качество сжатия для ускорения
                .downsample(com.bumptech.glide.load.resource.bitmap.DownsampleStrategy.AT_MOST); // Оптимизация downsampling
