...
import com.nonameradio.app.service.RadioMediaSessionService;
...
import com.nonameradio.app.core.event.TimerUpdateEvent;
import com.nonameradio.app.core.event.MeteredConnectionEvent;
...
public class PlayerService extends JobIntentService implements RadioPlayer.PlayerListener {
    ...
    public static final String PLAYER_SERVICE_TIMER_UPDATE = "com.nonameradio.app.timerupdate";
    public static final String PLAYER_SERVICE_META_UPDATE = "com.nonameradio.app.metaupdate";

    public static final String PLAYER_SERVICE_STATE_CHANGE = "com.nonameradio.app.statechange";
    public static final String PLAYER_SERVICE_STATE_EXTRA_KEY = "state";

    public static final String PLAYER_SERVICE_METERED_CONNECTION = "com.nonameradio.app.metered_connection";
    public static final String PLAYER_SERVICE_METERED_CONNECTION_PLAYER_TYPE = "PLAYER_TYPE";

    public static final String PLAYER_SERVICE_BOUND = "com.nonameradio.app.playerservicebound";
    ...
    /**
     * Update RadioMediaSessionService state to ensure UI synchronization
     */
    private void updateRadioMediaSessionState(int state) {
        try {
            // Get RadioMediaSessionService through MediaSessionManager
            NoNameRadioApp app = (NoNameRadioApp) getApplication();
            if (app != null) {
                // Use MediaSessionManager to update the shared MediaSession
                // This ensures the UI sees the same state as the notification
                Intent updateIntent = new Intent("com.nonameradio.app.UPDATE_MEDIASESSION_STATE");
                updateIntent.putExtra("state", state);
                updateIntent.putExtra("position", -1L);
                updateIntent.putExtra("speed", state == PlaybackStateCompat.STATE_PLAYING ? 1.0f : 0.0f);
                sendBroadcast(updateIntent);
                
                if (BuildConfig.DEBUG) {
                    Log.d(TAG, "Updated RadioMediaSessionService state to: " + state);
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Failed to update RadioMediaSessionService state", e);
        }
    }
    ...
    void sendBroadCast(String action) {
        // Migrated to EventBus - send appropriate event based on action
        switch (action) {
            case PLAYER_SERVICE_META_UPDATE:
                EventBus.post(MetaUpdateEvent.INSTANCE);
                break;
            case PLAYER_SERVICE_TIMER_UPDATE:
                EventBus.post(TimerUpdateEvent.INSTANCE);
                break;
            case PLAYER_SERVICE_STATE_CHANGE:
                // State changes are now handled via PlayerStateChangeEvent with actual state
                Log.d(TAG, "sendBroadCast: STATE_CHANGE (use PlayerStateChangeEvent instead)");
                break;
            default:
                Log.d(TAG, "sendBroadCast (unknown action): " + action);
        }
    }
    ...
    private final IPlayerService.Stub itsBinder = new IPlayerService.Stub() {
        ...
        public MediaSessionCompat.Token getMediaSessionToken() throws RemoteException {
            // Return our MediaSession token for UI integration
    ...
    public void onCreate() {
        ...
        // Use singleton RadioPlayer from NoNameRadioApp instead of creating new instance
        NoNameRadioApp application = (NoNameRadioApp) getApplication();
        radioPlayer = application.getRadioPlayer();
        radioPlayer.setPlayerListener(this);

        // Create our own MediaSession for proper state management
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            // Handle station and alarm parameters from MediaControllerHelper
            ...
            if (stationid != null) {
                ...
                NoNameRadioApp app = (NoNameRadioApp) getApplication();
                HistoryManager historyManager = app.getHistoryManager();
                DataRadioStation station = historyManager.getById(stationid);
                
                if (station == null) {
                    FavouriteManager favouriteManager = app.getFavouriteManager();
                    station = favouriteManager.getById(stationid);
                }
                
                if (station != null) {
                    currentStation = station;
                    playCurrentStation(isAlarm);
                    return START_STICKY;
                }
            }
                ...
                switch (action) {
                    ...
                    case ACTION_MEDIA_BUTTON:
                        ...
                        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
                            key = intent.getParcelableExtra(Intent.EXTRA_KEY_EVENT, KeyEvent.class);
                        } else {
                            key = (KeyEvent) intent.getParcelableExtra(Intent.EXTRA_KEY_EVENT);
                        }
    ...
    public void playCurrentStation(final boolean isAlarm) {
        if (BuildConfig.DEBUG) Log.d(TAG, "playCurrentStation called, isAlarm=" + isAlarm);
        ...
        // For alarms, try to play even if AudioFocus is not granted
        // For regular playback, require AudioFocus
        boolean shouldPlay = (result == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) || isAlarm;
        
        if (BuildConfig.DEBUG) Log.d(TAG, "AudioFocus result=" + result + ", shouldPlay=" + shouldPlay);
        
        if (shouldPlay) {
            if (BuildConfig.DEBUG) Log.d(TAG, "Starting playback...");
            // Start playback.
            enableMediaSession();
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "radioPlayer.play() called");
        } else {
            if (BuildConfig.DEBUG) Log.d(TAG, "Not starting playback - AudioFocus not granted and not alarm");
        }
    }
    ...
    private void setMediaPlaybackState(int state) {
        // Update RadioMediaSessionService via broadcast
        updateRadioMediaSessionState(state);
        
        // Also update local MediaSession if it exists
        if (mediaSession != null && mediaSession.isActive()) {
            ...
            if (BuildConfig.DEBUG) {
                Log.d(TAG, "MediaSession state updated directly: " + state);
            }
        } else {
            if (BuildConfig.DEBUG) {
                Log.w(TAG, "MediaSession is null or inactive, state not updated locally");
            }
        }
    }
    ...
    private void enableMediaSession() {
        if (mediaSession != null && !mediaSession.isActive()) {
            mediaSession.setActive(true);
            if (BuildConfig.DEBUG) Log.d(TAG, "MediaSession activated");
        }
    }
    ...
    private void disableMediaSession() {
        if (mediaSession != null && mediaSession.isActive()) {
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "MediaSession deactivated");
        }
        // No longer manage our own MediaSession
        // RadioMediaSessionService handles this
        if (BuildConfig.DEBUG) Log.d(TAG, "MediaSession is managed by RadioMediaSessionService");
    }

    private int acquireAudioFocus(boolean isAlarm) {
        if (BuildConfig.DEBUG) Log.d(TAG, "acquiring audio focus.");

        // Use STREAM_ALARM for alarms, STREAM_MUSIC for regular playback
    ...
    private void sendMessage(String theTitle, String theMessage, String theTicker) {
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "sendMessage called: " + theTitle + " - " + theMessage);
        }
        ...
        notificationBuilder.addAction(R.drawable.ic_skip_next_24dp, getString(R.string.action_skip_to_next), pendingIntentNext)
                .setStyle(new MediaStyle()
                        .setMediaSession(mediaSession != null ? mediaSession.getSessionToken() : null) // Use our own MediaSession
                        .setShowActionsInCompactView(1, 2, 3 /* previous, play/pause, next */)
                        .setCancelButtonIntent(pendingIntentStop)
                        .setShowCancelButton(true));
        
        // Log MediaSession token for debugging
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "MediaSession token: " + (mediaSession != null ? "available" : "null"));
        }
    ...
    private void updateNotification(PlayState playState) {
        switch (playState) {
            case Idle:
                // Don't cancel notification in Idle state - keep it for user control
                // Only cancel when explicitly stopped by user
    ...
    private void warnAboutMeteredConnection(PlayerType playerType) {
        ...
        // Post event via EventBus instead of LocalBroadcastManager
        EventBus.post(new MeteredConnectionEvent(playerType));
    ...
    public void onStateChanged(final PlayState state, final int audioSessionId) {
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                // Post event via EventBus instead of LocalBroadcastManager
                EventBus.post(new PlayerStateChangeEvent(state));
            }
        });
    }
