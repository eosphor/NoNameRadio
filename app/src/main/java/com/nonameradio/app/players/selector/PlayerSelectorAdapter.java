...
import com.nonameradio.app.players.mpd.tasks.MPDChangeVolumeTask;
import com.nonameradio.app.players.mpd.tasks.MPDPauseTask;
import com.nonameradio.app.players.mpd.tasks.MPDResumeTask;
import com.nonameradio.app.players.mpd.tasks.MPDStopTask;
...
public class PlayerSelectorAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private void bindViewHolder(@NonNull final PlayerItemViewHolder holder, int position) {
        if (holder.getItemViewType() == PlayerType.INTERNAL.getValue()) {
            ...
            holder.btnPlay.setOnClickListener(view -> PlayStationTask.playCAST(stationToPlay, context).executeAsync());
        }
    }
            ...
            dropDownMenu.setOnMenuItemClickListener(menuItem -> {
                switch (menuItem.getItemId()) {
                    ...
                    case R.id.action_play: {
                        PlayStationTask.playMPD(mpdClient, mpdServerData, stationToPlay, context).executeAsync();
                        break;
                    }
        ...
        if (holder.mpdServerData.connected) {
            if (stationToPlay != null) {
                ...
                if (mpdServerData.status != MPDServerData.Status.Playing) {
                    holder.btnPlay.setOnClickListener(view -> PlayStationTask.playMPD(mpdClient, mpdServerData, stationToPlay, context).executeAsync());
                } else {
